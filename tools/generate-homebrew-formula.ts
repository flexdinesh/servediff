...
function parseArgs(args: string[]): FormulaOptions {
  ...
  for (let index = 0; index < normalizedArgs.length; index += 2) {
    ...
    if (
      ...
      throw new Error(
        "usage: pnpm generate:homebrew -- --version <version> --tag <tag> --checksums <path> --output <path>",
      );
    }
...
async function generateFormula({
  ...
  tag,
  checksums,
}: FormulaOptions): Promise<string> {
  const checksumByArtifact = parseChecksums(await readFile(checksums, "utf8"));
  const encodedTag = encodeURIComponent(tag);
  const archives = targets.map((target): Archive => {
  ...
  const archiveByKey = new Map<string, Archive>(
    archives.map((archive) => [
      `${archive.homebrewOS}/${archive.homebrewArch}`,
      archive,
    ]),
  );
...
function parseChecksums(text: string): Map<string, string> {
  ...
  for (const line of text.split("\n")) {
    ...
    if (sha256 === undefined || artifact === undefined) {
      throw new Error(`invalid checksum line: ${line}`);
    }
    checksums.set(artifact, sha256);
  }
  return checksums;
}
